...
<body class="h-screen bg-main-light text-main-dark">
    ...
    <script>
        ...
        window.executarGeracaoCronograma = async function() {
            ...
            if (generateBtn && generateBtn.getAttribute('data-has-hours') === 'false') {
                if (typeof window.showNoHoursModal === 'function') {
                    window.showNoHoursModal();
                } else {
                    alert('Você não tem horas cadastradas no seu perfil! Vá até o perfil para configurar.');
                }
                return false;
            }
        ...
        // JS Tutorial and Modals setup
        function initTutorialAndModals() {
        ...
        // Initialize Tutorial and Modals on load and on turbo visits
        initTutorialAndModals();
        if (!window.tutorialModalsInitialized) {
            window.tutorialModalsInitialized = true;
            document.addEventListener('turbo:load', initTutorialAndModals);
        }
    </script>
</body>
